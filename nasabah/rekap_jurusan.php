...
$jurusan_query = $jurusan_filter !== '' ? "jurusan = '$jurusan_filter'" : '1=1';

// Filter pencarian nama
$search_query = isset($_GET['search']) ? $_GET['search'] : '';
$name_query = $search_query !== '' ? "nama LIKE '%$search_query%'" : '1=1';

// Kombinasikan filter status, jurusan, dan pencarian nama
$where_query = "WHERE $status_query AND $jurusan_query AND $name_query";

// Ambil jumlah total data dengan filter status, jurusan, dan pencarian nama
$total = $conn->query("SELECT COUNT(*) AS total FROM nasabah $where_query")->fetch_assoc()['total'];
$pages = ceil($total / $per_page);

// Tampilkan data dengan limit, offset, filter status, jurusan, dan pencarian nama
$q = $conn->query("SELECT * FROM nasabah $where_query LIMIT $start, $per_page");

// Ambil data jurusan untuk dropdown
$jurusan_result = $conn->query("SELECT DISTINCT nama FROM jurusan");
?>

<!DOCTYPE html>
<html>
<head>
    <title>Rekap Jurusan</title>
    <style>
        body {
    ...
    <div class="container">
        <div class="header">
            ...
            <div class="dropdown-wrapper">
                ...
                <div class="dropdown">
                    <button class="dropbtn">Filter Jurusan</button>
                    <div class="dropdown-content">
                        <a href="?jurusan=&status=<?= $status_filter ?>&search=<?= $search_query ?>">Semua</a>
                        <?php while ($jurusan = $jurusan_result->fetch_assoc()): ?>
                            <a href="?jurusan=<?= $jurusan['nama'] ?>&status=<?= $status_filter ?>&search=<?= $search_query ?>"><?= $jurusan['nama'] ?></a>
                        <?php endwhile; ?>
                    </div>
                </div>
        ...
        <div class="pagination">
            <!-- Tombol 'Sebelumnya' -->
            <?php if($page > 1): ?>
                <a href="?page=<?= $page - 1 ?>&status=<?= $status_filter ?>&search=<?= $search_query ?>&jurusan=<?= $jurusan_filter ?>">Sebelumnya</a>
            <?php endif; ?>
            ...
            <?php for($i = 1; $i <= $pages; $i++): ?>
                <?php if($i == $page): ?>
                    ...
                    <a href="?page=<?= $i ?>&status=<?= $status_filter ?>&search=<?= $search_query ?>&jurusan=<?= $jurusan_filter ?>" style="background-color: #333e50;"><?= $i ?></a>
                <?php else: ?>
                    <a href="?page=<?= $i ?>&status=<?= $status_filter ?>&search=<?= $search_query ?>&jurusan=<?= $jurusan_filter ?>"><?= $i ?></a>
                <?php endif; ?>
            <?php endfor; ?>

            <!-- Tombol 'Selanjutnya' -->
            <?php if($page < $pages): ?>
                <a href="?page=<?= $page + 1 ?>&status=<?= $status_filter ?>&search=<?= $search_query ?>&jurusan=<?= $jurusan_filter ?>">Selanjutnya</a>
            <?php endif; ?>
        </div>
    </div>
